...
describe('Core API tests', () => {
  ...
  it('GET video', () => {
    cy.request('GET', `${apiUrl}/videos/${sampleVideoId}`).then(response => {
  ...
  it('GET channel', () => {
    ...
    cy.request({ method: 'GET', url: `${apiUrl}/channels/${channelId}` }).then(response => {
      expect(response.status).to.eq(200);
  ...
  it('GET autocomplete result', () => {
    const query = 'john';
    cy.request({ method: 'GET', url: `${apiUrl}/autocomplete?q=${query}` }).then(response => {
      expect(response.status).to.eq(200);
  ...
  it('GET search result', () => {
    const q = 'Marques';
    cy.request({ method: 'GET', url: `${apiUrl}/search?q=${q}` }).then(response => {
      expect(response.status).to.eq(200);
    });
  });

  it('GET top comments', () => {
    cy.request({ method: 'GET', url: `${apiUrl}/comments/${sampleVideoId}` }).then(response => {
      expect(response.status).to.eq(200);
  ...
  it('GET playlist', () => {
    ...
    cy.request({ method: 'GET', url: `${apiUrl}/playlists/${playlistId}` }).then(response => {
      expect(response.status).to.eq(200);
