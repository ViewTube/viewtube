...
- [ ] **[high] Some videos stop after ~1 minute because BotGuard detects us as automation.** `is8UDe2PhKQ` and
      `Nz9b0oJw69I` carry `STREAM_PROTECTION_STATUS: 2` from the first SABR response and stop at 63.6s. **A real
      incognito Chrome plays them end to end and seeks freely**, so the video, the network and the egress are all fine —
      what differs is the client. A puppeteer-driven Chrome stops at 41s, which is the key clue: automation is detected
      regardless of whether a token is sent, and every PO token experiment that "failed" was using a token minted in an
      environment BotGuard had already rejected (jsdom, or an automated browser).
  - So `common/potoken/` is the right subsystem with the wrong minter. The open question is what minting environment
    BotGuard accepts — not whether the token matters.
  - Not retryable in the player: a completely fresh session with a new token resumes to the same second
    (`npm run resume`).
  - Full working through, including the two superseded diagnoses and why each was wrong, in `SABR_PLAN.md`.
- [wontfix] **~~Some videos are restricted on some egress addresses~~** — superseded by the entry above; the browser
  control that supported it was itself being detected as automation. What is still true is that the _failure mode_
  varies by exit: some refuse these videos outright as `UNPLAYABLE` rather than cutting them off partway. `is8UDe2PhKQ` and `Nz9b0oJw69I` stop partway
  (`STREAM_PROTECTION_STATUS: 2`, media ends ~43-63s) from one VPN exit, and are refused outright as `UNPLAYABLE` with
  no SABR endpoint from another, while `dQw4w9WgXcQ` plays fine from both. **Chrome on youtube.com fails identically
  from the same network** — 42.6s of a 153s video, then YouTube's own "Something went wrong" — so there is no ViewTube
  defect and no client change that helps. Cookies make no difference (accept 42.6s vs reject ~43s). The player reports
  it honestly and stops there.
  - Before treating a mid-playback stall as a bug again, run the browser control first:
    `npm run timeline -- --video <id> --consent accept`. Skipping it is what made this look like a protocol bug for a
    day, and produced a wrong "attestation gate" diagnosis in `SABR_PLAN.md`.
  - **A fresh session mid-video does not resume it** (`npm run resume`, 2026-08-28). A completely independent session —
    new player request, new streaming URL and ustreamer config, newly minted PO token — stops at exactly the same
    63.6s, having received a byte-identical amount. The cut-off is anchored to the position in the video, not to the
    age of the session, so wiring the wall into `sabrAdapter`'s existing `reloadSession` path would not help.
  - Not the egress either: the same 63.6s happens on a residential IP with no VPN, and `STREAM_PROTECTION_STATUS` is
    1 vs 2 per _video_ on every network tried. What varies by exit is only whether the video is refused earlier
    (`UNPLAYABLE`, no stream at all) instead of partway.
...
- [ ] **[low] The PO token is minted but nothing proves YouTube reads it.** `common/potoken/` mints and ships a
      correctly bound per-video token, and it demonstrably does not change the attestation gate. No probe distinguishes
      "YouTube accepted it" from "YouTube ignored it", so the value of the whole subsystem is currently assumed rather
      than measured. `STREAM_PROTECTION_STATUS` staying at 1 on unflagged videos is the only positive signal we have, and
      those play without a token too. Worth a probe that flags a _fresh_ IP with and without tokens over time — that is
      the case tokens are supposed to help with.
...
- [x] **[scale] ~~`VIEWTUBE_PROXY_URL` is unvalidated~~** — added to `env.validation.ts` 2026-08-27 alongside
      `VIEWTUBE_POTOKEN_ENABLED`. Still undocumented outside `CLAUDE.md`; the deployment-topology entry above is where
      that belongs.
